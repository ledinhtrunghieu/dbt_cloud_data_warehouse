...
WITH PreDrugTarget AS(
    ...
    FROM `bachelor-thesis-344103`.`optimized_data_warehouse`.`drug_exposure` d
        INNER JOIN `bachelor-thesis-344103`.`transformed_data_warehouse`.`concept_ancestor` ca ON ca.descendant_concept_id = d.drug_concept_id
        INNER JOIN `bachelor-thesis-344103`.`transformed_data_warehouse`.`concept` c ON ca.ancestor_concept_id = c.concept_id
